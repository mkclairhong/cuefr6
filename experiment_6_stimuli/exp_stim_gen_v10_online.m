...
%% Stimuli creation for for Experiment 6 
% base wordpool = Polyn et al. (2011)
...
for k = 2100:2120
    ...
    stimfile = strings([LL*n_session+1,10]);
    sub_num = num2str(k);
    headers = {'suject', 'session', 'trials', 'category', 'target', 'rp', ' ', 'critical_cat', 'critical_target', 'critical_rp'};
    ...
    trials = [];
    trials(1      :LL,1)    = (1:1:LL);
    trials(LL+1   :LL*2,1)  = (1:1:LL);
    trials(LL*2+1 :LL*3,1)  = (1:1:LL);
    trials(LL*3+1 :LL*4,1)  = (1:1:LL);
    ...
    for i = 1:n_session
        ...
        % 1st column = subject number
        stimfile(1,1) = headers(1);
        stimfile(2:101,1) = sub_num;
        
        % 2nd column = session number 
        stimfile(1,2) = headers(2);
        stimfile(2:101,2) = sessions; % 1 through 100 because 25 words/session * 4 sessions
        
        % 3rd column = trial number
        stimfile(1,3) = headers(3)';
        stimfile(2:101,3) = trials;
        
        % 4nd column: category
        stimfile(1,4) = headers(4);
        stimfile(LL*(i-1)+2         : LL*(i-1)+LFB+1,4)         = filler_cat(1:5,i); % rows 2 to 6
        stimfile(LL*(i-1)+2+LFB     : LL*(i-1)+LFB+LC+1,4)      = stim_cat(1:18,i);
        stimfile(LL*(i-1)+2+LFB+LC  : LL*(i-1)+LFB+LC+LFE+1,4)  = filler_cat(6:7,i);
       
        % 5th column: target
        stimfile(1,5) = headers(5);
        stimfile(LL*(i-1)+2         : LL*(i-1)+LFB+1,5)         = lower(filler_target(1:5,i));
        stimfile(LL*(i-1)+2+LFB     : LL*(i-1)+LFB+LC+1,5)      = lower(stim_target(1:18,i));
        stimfile(LL*(i-1)+2+LFB+LC  : LL*(i-1)+LFB+LC+LFE+1,5)  = lower(filler_target(6:7,i));
       
        % 6th column: RP
        stimfile(1,6) = headers(6);
        stimfile(LL*(i-1)+2         : LL*(i-1)+LFB+1,6)         = lower(filler_rp(1:5,i));
        stimfile(LL*(i-1)+2+LFB     : LL*(i-1)+LFB+LC+1,6)      = lower(stim_rp(1:18,i));
        stimfile(LL*(i-1)+2+LFB+LC  : LL*(i-1)+LFB+LC+LFE+1,6)  = lower(filler_rp(6:7,i));
        
        % 8th column: critical category
        stimfile(1,8) = headers(8);
        stimfile(LL*(i-1)+2+LFB     : LL*(i-1)+LFB+LC+1,8) = stim_cat(1:18,i);
        
        % 9th column: critical target
        stimfile(1,9) = headers(9);
        stimfile(LL*(i-1)+2+LFB     : LL*(i-1)+LFB+LC+1,9) = lower(stim_target(1:18,i));
        
        % 10th column: critical RP
        stimfile(1,10) = headers(10);
        stimfile(LL*(i-1)+2+LFB     : LL*(i-1)+LFB+LC+1,10) = lower(stim_rp(1:18,i));
    end
    
    stim_export_dir = strcat(pwd,'/stimuli_online'); 
    subj = num2str(k);
    ...
    output_name = sprintf([pwd,'/stimuli_online/','stim_subj_%d'],k);
